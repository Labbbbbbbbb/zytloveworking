...
float position[2]={0};  //及0=Xb 和 1=Yb
float Encoder0[2]={0};
float Encoder1[2]={0};
void Error_Modify(float error_angle)  //单位是弧度,钝角时取负值
{
    delta_distance[0]*=(cos(error_angle)+sin(error_angle));
    delta_distance[1]*=(cos(error_angle)+sin(error_angle));
}
//**待定：编码器转化成距离的方法 */
void Timing_Distance_Calculate(void)
{
    ...
    float yaw_aver=(yaw_temp+fAngle[2]-offset_yaw)/2/180*3.14159;
    tag=yaw_aver;
    //**计算出distance，即为单位时间内的载体坐标系位移**//
    //printf("yaw_aver:%f,%f\n",yaw_aver,fAngle[2]);
    Encoder1[0]=angle1-offset_angle[0];
    Encoder1[1]=angle2-offset_angle[1];
    delta_distance[0]=Encoder1[0]-Encoder0[0];
    delta_distance[1]=Encoder1[1]-Encoder0[1];
    ...
    delta_distance[0]*=(float)R1/180*3.14159;
    delta_distance[1]*=(float)R2/180*3.14159;
